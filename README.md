...
- 普通主题检索
  - 优先用 `search_petroleum_literature`
  - 适合“测井解释”“地震反演”“储层评价”“页岩气”等
- 指定来源检索
  - 用 `search_cnki`、`search_geophysics`、`search_spe` 这类单源工具
- 学者 / 老师检索
  - 优先用 `search_scholar_publications`
  - 尤其适合“张凯 + 中国石油大学 + 声波测井”这类查询
  - 方向词越具体，效果通常越好，例如 `acoustic logging` 往往优于泛化的 `logging`
- 最新论文
  - 把 `sortBy` 设为 `published`
  - 可配合 `yearFrom` / `yearTo`
